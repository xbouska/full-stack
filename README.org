...
**** MongoDB
**** Apache Drill
*** 5.1.1 inner vs outher join  typy spojení DB tabulek, inner  výsledek musí obsahovat průnik obou tabulek, outer  výsledek obsahuje data z jedné či druhé, případně z obou tabulek pokud klíč existuje v obou tabulkách …  záleží na typu outer joinu - Full/left/right
5.1.2 partitionning 
...
*** Agregace  group by
*** Co jsou window funkce?  
- kalkulace/agregace nad množinou dat, přičemž každý prvek množiny může mít výsledek agregace jiný …. Např funkce lead/lag/rank/row_number/first/last/
*** CTE (with)  
- syntaxe pro vytvoření dočasné tabulky, kterou mohu použít v následujícím sql dotazu, lze pomocí ní dělat rekurzivní dotazy, výsledek je jen v paměti a neukládá se na disk (pokud mám dostatek working memory 😊 )
*** DML/DDL
- manipuklace s daty v tabulce (insert/delete/update) / definuje datové struktury (db,schema, tabulka) (create / drop / alter / rename)
*** Pojem datawarehouse?  
- úložiště dat, organizovaných do určité struktury, pro účely analýzy dat … podpora business procesů organizace
*** Star schema  DB schema 
...
WAL  možná bych dal obecně transakční log?  log kde je historie všech provedených DB operací nad danou instancí/databází. Z logu se zapisují záznamy do datových souborů. Z logu se dá obnovit databáze do určitého časového okamžiku (PITR-Point In Time Recovery); u postgre se dá použít například i k replikaci databáze
